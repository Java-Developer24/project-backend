...
const calculateUpdatedPrice = (price, discount = 0) => {
    const discountedPrice = parseFloat(price) + discount;
    return parseFloat((discountedPrice * 1.3 + 1).toFixed(2));
};
    ...
    try {
        ...
        for (const serviceData of servicesData) {
            ...
            const parsedServers = servers.map(server => {
                ...
                const updatedPrice = updatedPriceFormula(price); // Apply the formula to calculate updated price

                // Round the updated price to 2 decimal places
                const roundedPrice = parseFloat(updatedPrice.toFixed(2)); // Round to two decimal places

                // Update the price directly
                return {
                    ...rest,
                    serverNumber,
                    price: roundedPrice // Set the rounded price directly
                };
            });
...
// Normalize the service name (as you have already defined)
...
const marginAmt = 1; // Example margin amount, adjust as necessary
const getPricingFormula = (url, serverNumber) => {
    const multiplier = [3, 9, 10, 11].includes(serverNumber) ? 95 : 1.1; // Multiplier for specific servers
    
    return (price) => multiplier * price + marginAmt; // Pricing formula
};
